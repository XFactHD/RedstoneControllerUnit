...
public final class RedstoneConfig {
    ...
    private static final ColorCollection<Component> TOOLTIP_WIRE_COLOR = Utils.COLOR_NAMES.map(
            name -> Component.translatable(TOOLTIP_WIRE_COLOR_KEY, name)
    );
                ...
                case BundledPortConfig bundle -> {
                    ...
                    for (int i = 0; i < 8; i++) {
                        ...
                        if (mouseX >= px && mouseX < px + WIDTH_BUNDLE_BIT) {
                            DyeColor color = DyeColor.byId(i + (bundle.upper() ? 8 : 0));
                            var lines = List.of(TOOLTIP_PORT_BIT[i].getVisualOrderText(), TOOLTIP_WIRE_COLOR.pick(color).getVisualOrderText());
    ...
    public boolean mouseClicked(MouseButtonEvent event, PortConfig[] configs) {
        if (event.button() == InputConstants.MOUSE_BUTTON_LEFT && event.x() >= x + X_TYPE && event.x() < x + WIDTH && event.y() >= y && event.y() <= y + HEIGHT) {
